...
> 表单的**编辑链接**和**报名数据表链接**不放在本仓库，存在项目的私有资料目录里。
> 它们不是密码——权限由 Google 控制，已实测匿名无法读取，含 CSV 导出——但数据表
> 里有未成年人的出生日期和家长联系方式，把地址公开等于对外宣告这份资料的存在。
> 万一日后有人为图方便把共享改成「知道链接的任何人」，地址早已在外。
