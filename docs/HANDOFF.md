...
>
> 最后更新：2026-08-16（导航精简、年龄自动推算、脚本转英文后）
>
> 单次会话的详细记录在 `worklog-*.md`。
...
| **性质** | 志愿项目，无偿 |
| **赛事报名** | https://gag.golf/register |
| **会员加入** | https://gag.golf/membership |
...
### 已完成的页面（英法双语，共 20 个路由）

| 路由 | 内容 |
|---|---|
| `/` `/fr` | Hero、GAG 是什么、四项主张、近期赛事、三大板块入口 |
| `/membership` `/fr/membership` | **会员（免费）**：现在意味着什么、将来会有什么、与赛事报名的区别 |
| `/tournaments` `/fr/tournaments` | 首届赛事详情、三大常设赛事、未来赛事、赛事页将包含的栏目 |
...
**开放/关闭赛事报名** → 改 `src/config/site.ts` 的 `registrationUrl`。设为 `null` 时页面自动显示「报名尚未开放」，不会留死链。

**开放/关闭会员加入** → 改 `src/config/site.ts` 的 `membershipUrl`，同上。

> 表单字段、表格结构、隐私设置、邮件与会员号配置全在 [`REGISTRATION.md`](REGISTRATION.md)，脚本在 `docs/*.gs`。
...
**9. 会员与赛事报名是两套，不要合并。** 会员 = 长期身份、免费、人人可加；报名 = 单场比赛、有差点门槛、会员与否都能报。客户明确要求区分，ACBL 等成熟组织也都这么分。`/membership` 页面最后一节专门讲这个区别——访客搞混的话，要么以为加入了就有位置，要么以为不加入就不能打。

**10. 会员页不承诺任何权益。** 现阶段免费且无权益，页面就照实说，「将来会有什么」标注为意向而非承诺。**一个免费会员制度如果暗示了不存在的好处，最先受伤的是头一百个会员**——而他们正是在还没有任何理由时就加入的人。

**11. 同一个目的地在页头只出现一次。** Membership 只有右上角的 Join 按钮，不再进主导航——两个入口指向同一页，会让人以为是两个不同的东西。页脚是站点地图，那里该列全。

**12. 球员隐私单独成块，不塞进页脚。** 平台要公开 16 岁起球员的资料，决定要不要参加的是他们的家长和教练——这些人应该一眼看到承诺，而不是去翻法律条款。

**13. 页头只放三个目的地。** About · Tournaments · Contact + 语言切换 + JOIN 按钮（客户 2026-08-16 要求）。其余页面在 `secondaryNav`，由页脚列全——从页头拿掉不等于删除，无法导航到的页面等于没人看。

**14. 称呼统一用 "golf players"，不用 "golfers"。** 客户要求。改的时候是全文替换而非只改标语，否则标题换了词、两行下的正文还在用旧词。法文对应 `joueurs de golf`。

**15. 年龄一律从出生日期推算，永远不要再问。** 表单收了生日又问"是否未满18岁"，迟早收到两个互相矛盾的答案。年龄按**赛事当天**算（下周满 18 的球员本场仍是未成年人）。⚠️ **实现必须用触发器写数值，不能用表格公式**——数组公式放第 2 行会被表单的行复制顶掉（`#REF!`），放第 1 行会被「表格」表头拒绝。两个坑都实际踩过，别再试。

**16. 报名不要求会员身份。** 表单顶部明说 membership is not required。选填的 `GAG member number` 字段是两套登记的关联键，也是将来迁移到专业系统的主键。

**17. 登录/账号系统：买现成平台，不自建。** 存未成年人住址+生日+密码的系统，安全与合规责任不该由志愿项目承担。待客户试用 Wild Apricot / Golf Genius 后拍板；官网只负责把按钮指过去。
...
- ~~「Events」这个词~~ → 客户要求改为 **Tournaments**（英）/ **Tournois**（法），路由从 `/events` 移到 `/tournaments`，旧地址 301
- ~~表单里「是否未满 18 岁」的问题~~ → 已删除，由出生日期自动推算（决策 15）
- ~~会员表单收「年龄段」~~ → 新版脚本收出生日期；**现网表单未重建**（会丢已发出的会员号），沿用旧问法
